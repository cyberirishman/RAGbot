# RAG Chatbot v2 — Local Buildout Plan & Lesson Guide (16GB Teaching Edition)
...
## Why v2 exists (Hardware Target & Rationale)

This document is a **fork of v1.7** of the original Chatbot project, adapted so that the entire build can be run, taught, and demonstrated on a stock 16GB Apple Silicon Mac. The vast majority of students, staff, and entry-level technicians do not have a 64GB Mac mini. v1 was built and verified on one — v2 makes the same project reachable on the laptops and minis people actually own.

**What changes vs v1.7:**
- **Chat + vision model** — v1 uses `gemma4:26b` (~17GB on disk, needs ~25GB RAM headroom while loaded). v2 uses **`gemma4:e4b`** — Gemma 4's "effective 4 billion parameter" variant, ~10GB on disk, designed via MatFormer-style selective parameter activation to run on consumer hardware. It is multimodal (vision + text + tools + thinking, with audio and cloud capabilities advertised), so the same one model still handles BOTH image description during ingest AND chat responses. The 2-model design (`nomic-embed-text` + multimodal Gemma 4) is preserved.
- **Hardware specs** — minimum disk free drops 25GB → 15GB. RAM target drops 64GB → 16GB. New Pre-Flight RAM check added.
- **Vision quality disclosure** — diagram and chart description quality on `gemma4:e4b` is meaningfully lower than on `gemma4:26b`. This is acceptable for teaching and for most text-heavy field-service Q&A. For production-grade vision over real customer manuals, the recommended path is: run ingest *once* on the v1 architecture (64GB machine) and ship the resulting `chroma_db/` folder to a v2 deployment, OR rely on the cloud LLM in production via AWS Bedrock / Azure OpenAI as already described in `Enterprise_architecture.md`.
- **Common Pitfalls section** — adds a new entry for swap-thrashing symptoms on 16GB machines and how to recognise + mitigate them.

**What stays the same as v1.7:** every other architectural choice — Docling for parsing, ChromaDB for vectors, LangChain for orchestration (LCEL chain), Gradio for UI, `nomic-embed-text` for embeddings, two-stage chunking with `MarkdownHeaderTextSplitter`, conversation memory via injected history, streaming responses, rotating logs, deterministic chunk IDs, idempotent ingestion, all package version pins. The Python code in `src/` is identical between v1 and v2 — only `config.py`'s `CHAT_MODEL` value differs.

**Migration provenance:** v2.0 inherits all fixes and decisions accumulated in v1.0 → v1.7 (eight versions of refinement). See `MIGRATION_FROM_V1.md` for the full lineage. The historical changelog blocks below are preserved verbatim from v1.7 because every fix and lesson in them still applies to v2.

---

**Changes in v2.7 (Architecture Overview diagram split into Ingest + Query + Shared Infrastructure — 2026-05-07):**
- **Single linear arrow-chain replaced with two pipeline diagrams plus a shared-infrastructure block.** The previous "six-layer assembly line" diagram conflated ingest steps (Docling → vision → embedder → ChromaDB write) with query steps (LangChain → retrieve → LLM → Gradio) on a single arrow chain, which inadvertently suggested the chatbot performed all of those things every time the user asked a question. Reality is the opposite: ingest runs once per document (slow, batch — minutes per document), query runs once per user message (fast, online — 2–8 seconds per answer). Splitting them clarifies what is expensive vs what is per-message, and surfaces Ollama and ChromaDB as the shared infrastructure that both pipelines depend on.
- **New Ingest Pipeline diagram** shows: source documents → Docling parse → branching text/image paths (text into the two-stage chunker; images into `gemma4:e4b` for description, with the description rejoining the text path as its own chunk) → `nomic-embed-text` embedder (768-dim vectors) → ChromaDB write → source file moved to `ingest/processed/` for idempotency.
- **New Query Pipeline diagram** shows: Gradio browser input → `src/app.py` `chat_handler` (Gradio generator with per-tab history) → `src/chat.py` LCEL chain → `nomic-embed-text` (question → vector) → ChromaDB similarity search (top-K=5) → LCEL prompt assembly (system instructions + last `HISTORY_TURNS` messages + retrieved chunks + user question) → `gemma4:e4b` streaming answer → tokens back to Gradio. Surfaces both `HISTORY_TURNS` (v2.5) and the LCEL chain explicitly.
- **New Shared Infrastructure block** below the two pipelines names Ollama (HTTP service serving both models, with the `OLLAMA_KEEP_ALIVE` keep-loaded behaviour) and ChromaDB (embedded mode, persisted on disk, built once / queried many times).
- **Intro paragraph rewritten.** The old "six layers that work together like an assembly line" framing was replaced with a paragraph that explicitly states the two-pipeline structure, the two pieces of shared infrastructure, and the cost asymmetry between batch ingest and online query. This is a teaching-clarity improvement requested by Andrew while preparing to record course content.
- **What did NOT change.** The follow-on paragraphs immediately after the diagrams — "2-model design (preserved from v1.4 → v2.0)", "Vision quality disclosure", "The key concept — RAG", and the "RAM footprint (16GB Apple Silicon Mac)" table — are unchanged. The folder-structure block elsewhere in the document is also unchanged.
- **No source-code changes.** `src/*.py`, `config.py`, and `requirements.txt` are unchanged. The v2.6 Step 11.8 / Pitfall 10, the v2.5 `HISTORY_TURNS` fix, and the v2.4 docling fix are all preserved.

**Changes in v2.6 (operational hygiene — clean termination of `app.py` and Ctrl-Z recovery — 2026-05-07):**
- **New Step 11.8 — "Ending the RAG retrieval process after testing/use"** inserted between the existing 11.7 ("Test with known questions") and what was previously 11.8 ("Check the log file"). Explains that `python src/app.py` runs both the Gradio web server (the chat UI on `http://localhost:7860`) and the RAG retrieval pipeline (embedding, ChromaDB search, prompt building, `gemma4:e4b` inference) in the same Python process — terminating `app.py` stops both. Documents that **Ctrl-C** is the correct termination key (clean shutdown via SIGINT) and **Ctrl-Z** is NOT (it suspends rather than terminates — see Pitfall 10).
- **Old Step 11.8 ("Check the log file") renumbered to Step 11.9.** No content change to that step.
- **New Pitfall 10 — "`zsh: suspended  python src/app.py` after pressing Ctrl-Z"** added after Pitfall 9. Documents the symptom (process still in `ps`, port 7860 still bound, ~10 GB RAM still committed), the SIGTSTP-vs-SIGINT root cause, and three recovery options (`fg` then Ctrl-C, `kill <PID>` with optional `kill -9` escalation, or `jobs` then `kill %1`). Cross-references Pitfall 9 (swap thrashing) because a forgotten suspended process is one of its most common triggers on 16GB hardware.
- **Why it earned a release.** The Ctrl-Z trap surfaced on Andrew's dry-run machine on 2026-05-07 right after the U-30 verification. It is exactly the kind of stumbling block a Windows-background admin learning Unix shell semantics will hit, so it deserves a permanent home in the canonical plan rather than living only in a session transcript.
- **No source-code changes.** `src/*.py`, `config.py`, and `requirements.txt` are unchanged. The v2.5 `HISTORY_TURNS` fix and the v2.4 docling fix are preserved.

**Changes in v2.5 (HISTORY_TURNS config-constant fix back-ported into the canonical heredoc — 2026-05-07):**
- **`config.py` heredoc fixed** — Phase 7's heredoc was missing `HISTORY_TURNS = 6`. The constant is referenced by `src/chat.py`'s `format_history()` helper at `recent = history[-config.HISTORY_TURNS:]`. A fresh student following the plan literally would build a `config.py` that doesn't expose `HISTORY_TURNS`, then crash on the *second* user message (every follow-up query) with `AttributeError: module 'config' has no attribute 'HISTORY_TURNS'`. The first message survives because `format_history()` short-circuits with the "No prior conversation." placeholder before reaching the broken line. Verified hitting this on Andrew's dry-run machine on 2026-05-07 during Phase 11 end-to-end testing.
- **Root cause is the U-29 pattern repeated.** v1.5's changelog narrative explicitly named the new constant ("added `HISTORY_TURNS = 6` to `config.py` (Retrieval Settings section)") but the v1.5 author updated only the narrative, never the actual heredoc. v1's `config.py` likely carries the same latent gap; v2 inherited the heredoc unchanged through v2.4.
- **Two coherent edits to the heredoc** to make it correct from a fresh paste: (1) added a new `# --- Conversation Memory Settings ---` block between the existing Retrieval Settings and Vision Model Context Window blocks, with `HISTORY_TURNS = 6` and a multi-line teaching comment that explains what it controls, that history is browser-tab-scoped, and that the constant is referenced by `chat.py`'s `format_history()`; (2) added a new `> **New in v2.5 — HISTORY_TURNS = 6**` teaching note below the heredoc paired with the existing `> **New in v1.x**` lines, summarising the back-port and pointing at U-30 for the full root cause.
- **Verified working on the target machine (2026-05-07).** Andrew copied the new `# --- Conversation Memory Settings ---` block from the v2.5 Phase 7 heredoc into live `config.py` (at the project root, not under `src/`), restarted `python src/app.py`, asked a first question that returned a grounded answer, then asked a pronoun-style follow-up — the follow-up answered normally instead of crashing with the previous `AttributeError`. Startup banner printed all five lines, knowledge-base check reported 12 chunks, Gradio came up on `http://0.0.0.0:7860`. Unrelated startup noise (ChromaDB anonymous-telemetry `capture()` warnings and Gradio 6.0 forward-looking deprecation warnings) appeared but is harmless and documented separately.
- **No live `config.py` edit in this v2.5 release on the project side.** Andrew applies the patch by manually copying from the heredoc into the live file as part of the dry-run-then-virgin-machine workflow. The heredoc IS the canonical source; the live file is now in sync with it.
- **v1 latent bug flag.** `Chatbot/` v1.7's `config.py` was the original source of v2's `config.py`, so v1 likely carries the same latent `HISTORY_TURNS` gap. v1 is not modified from v2 sessions, so this v2.5 changelog flags it for any future v1 session: the same one-block fix can be back-ported there if needed. v1's existing working environment likely has not been restarted from a fresh shell since v1.5 was authored, so the live `config.py` on that machine *might* have been edited in place — this is unverified from a v2 session.
- **No source-code changes anywhere else.** `src/chat.py`, `src/app.py`, `src/ingest.py`, `src/logging_setup.py`, and `requirements.txt` are unchanged — none of them needed to change. The v2.3 venv name (`chatbot-16G-venv/`) and the v2.4 docling import-fix are preserved.

**Changes in v2.4 (docling.__version__ fix back-ported into the canonical heredoc — 2026-05-07):**
- **`src/ingest.py` heredoc fixed** — the Phase 8 heredoc that builds `src/ingest.py` previously contained `import docling` plus `docling.__version__` in the startup version banner. Docling 2.14.0 does not expose `__version__` as a top-level module attribute, so any fresh student following the plan literally would hit `AttributeError: module 'docling' has no attribute '__version__'` and crash before any actual ingestion ran. Verified hitting this on Andrew's dry-run machine on 2026-05-07.
- **Three coherent edits to the heredoc** to make it correct from a fresh paste: (1) added `import importlib.metadata` to the stdlib import block (alongside `platform` and `subprocess`); (2) removed the bare `import docling` line and its old comment, replacing the gap with an explicit comment block explaining why there is no bare `import docling` and why the absolute submodule imports (`from docling.document_converter import …` etc.) still work without it; (3) changed the version-banner line from `docling.__version__` to `importlib.metadata.version("docling")`, with inline comments above the LangChain/ChromaDB lines (which DO expose `__version__`) and above the Docling line (which doesn't) so the student sees the difference explicitly.
- **"Known Fixes — Fix 1" prose block reframed** as historical / pedagogical context. Previously titled "Known Fixes applied to ingest.py during first test run (2026-05-01)" — that framing implied the heredoc above still needed patching. After v2.4 the heredoc IS the post-fix canonical state, so the prose block now reads as the post-mortem explanation of why the heredoc looks the way it does. No information lost; the rationale and the specific Docling 2.14.0 behaviour stay documented for any reader who wonders.
- **Verification on the target machine.** Andrew copied the v2.4 heredoc into live `src/ingest.py` on the 16GB dry-run machine, re-ran `python src/ingest.py` from the activated `chatbot-16G-venv`, and the version banner now prints all five lines (Python, LangChain, ChromaDB, Docling, Ollama) and the script proceeds into the actual ingestion loop. Banner crash gone.
- **No live `src/ingest.py` edit in this v2.4 release on the project side** — Andrew applied the patch by manually copying from the heredoc into the live file as part of his dry-run-then-virgin-machine workflow. The heredoc IS the canonical source; the live file is now in sync with it.
- **v1 latent bug flag.** `Chatbot/` v1.7's `ingest.py` was the original source of v2's `ingest.py`, so v1 likely carries the same latent `docling.__version__` bug. v1 is not modified from v2 sessions, so this v2.4 changelog flags it for any future v1 session: the same three-line fix can be back-ported there if needed. Until then, v1 only fails this banner if it ever upgrades or reinstalls Docling on a fresh machine — its existing v1 working environment likely had `docling` already imported at runtime in a way that masked the issue, OR the v1 student never re-ran ingest from a fresh shell.
- **No source-code changes anywhere else.** `src/chat.py`, `src/app.py`, `src/logging_setup.py`, `config.py`, and `requirements.txt` are unchanged — none of them touch `docling.__version__`. The v2.3 venv name (`chatbot-16G-venv/`) is preserved.

**Changes in v2.3 (renamed venv folder for student clarity — 2026-05-07):**
- **Virtual environment folder renamed** from the generic `venv/` to **`chatbot-16G-venv/`**. The new name encodes the project (`chatbot`) and the hardware target (`16G`, for the 16GB Teaching Edition) directly into the folder name. This makes it obvious at a glance what the folder is for, prevents collisions if a student ever clones the v1 64GB project alongside this one, and keeps the activation prompt prefix self-documenting (`(chatbot-16G-venv)` instead of an ambiguous `(venv)`).
- **Phase 5 updated** — the create command is now `python3.12 -m venv chatbot-16G-venv` and the activate command is now `source chatbot-16G-venv/bin/activate`. The expected post-activation prompt prefix the student should look for is now `(chatbot-16G-venv)` (stock macOS `python -m venv` behaviour — no custom shell config required).
- **All later activation references updated** — every `source venv/bin/activate` in Phase 11 (Day-to-Day Operating Guide), the Restart-After-Reboot block, the Operate-Daily reminder, and inside Common Pitfalls 1, 8, and the cross-references in the rest of the doc now use `source chatbot-16G-venv/bin/activate`. Every "verify your prompt shows `(venv ...)`" check now reads `(chatbot-16G-venv)`.
- **Architecture-overview folder tree** now shows `├── chatbot-16G-venv/` instead of `├── venv/`, with the same comment ("Python virtual environment — never edit manually").
- **Quick-reference table at the bottom of the doc** updated to point students at `source chatbot-16G-venv/bin/activate`.
- **No source-code changes.** `src/*.py`, `config.py`, and `requirements.txt` are venv-name-agnostic — they activate within whichever venv is currently sourced and never refer to the folder name. `OLLAMA_BASE_URL = http://0.0.0.0:11434` preserved exactly.
- **Historical v1.6 changelog narrative left intact.** The line "venv not active, ChromaDB telemetry warnings, Ollama connection refused..." in the v1.6 entry below is a historical record of what shipped in v1.6 itself; rewriting it would falsify the lineage, so the v1.6 wording is preserved verbatim. Forward-looking instructions and the Pitfalls section are the canonical source of truth for current students.

**Changes in v2.2 (single dependency file — 2026-05-07):**
- **Collapsed `requirements.txt` and `requirements.lock.txt` into a single `requirements.txt`** containing all 182 transitively-resolved package pins from the verified-working environment. Eliminates the two-file split and the "which file does pip read?" confusion that was present in v1.6 → v2.1. `pip install -r requirements.txt` is now the only install command. The lockfile-first hardening from v1.6 is preserved in spirit — every package is still pinned, so pip's resolver still has no choices to make on a fresh install — but only one file ships in the project.
- **Phase 6 simplified.** Section header changed from "Create the Requirements File" to "Install Python Dependencies" because the file ships with the project. Step 6.1 is now a "confirm `requirements.txt` is in place" check; the previous curated-12-pin heredoc is replaced with the full 182-pin canonical list as a reference block (only needed if the file is ever lost). Step 6.2 is `pip install -r requirements.txt` with no two-file narrative. Step 6.3 (regenerate after upgrade) writes `pip freeze > requirements.txt` instead of `pip freeze > requirements.lock.txt`.
- **Common Pitfall 8 retitled** from "`pip freeze > requirements.lock.txt` produces only 3 lines" to "`pip freeze > requirements.txt` produces only 3 lines" — same diagnostic, single-file naming.
- **Folder-structure block** in the Architecture Overview no longer lists `requirements.lock.txt` separately.
- **No source-code changes.** `config.py`, `src/*.py` are untouched. `OLLAMA_BASE_URL = http://0.0.0.0:11434` preserved.
- **Pedagogical trade-off (recorded explicitly):** the previously inline rationale for direct dependencies — particularly the v1.5.1 gradio/Pydantic fix and the v1.6 defensive transitive pins — no longer appears as inline comments in `requirements.txt`. That educational content is preserved in this changelog and in `MIGRATION_FROM_V1.md`.

**Changes in v2.1 (Ollama install + service start — 2026-05-06):**
- **New Phase 1 — Install and Start Ollama** added between Phase 0 (Homebrew + Git) and the model-pull phase. Covers: (1) checking whether Ollama is already on the `PATH` with `ollama --version`, (2) installing it via the Homebrew **formula** (`brew install ollama`) — explicitly *not* the cask `.app` version, because the formula is a single system-level binary that any user on the machine can talk to via `OLLAMA_BASE_URL = http://0.0.0.0:11434`, while the cask runs as a per-user menu-bar app and would fight other users on the same Mac for port 11434, (3) starting the daemon with `brew services start ollama` so it auto-starts on every login, (4) verifying the daemon with `ollama list`, and (5) a teaching tip — `tail -f /opt/homebrew/var/log/ollama.log` to watch live API traffic, useful for showing students how the chatbot actually talks to Ollama. Future-session start/stop/status commands are documented in the same phase.
- **Phase numbering shifted by 1.** What used to be Phase 1 (Pull Required Ollama Models) is now Phase 2; old Phase 2 through Phase 11 each shifted up by 1, and old Phase 6.5 is now Phase 7.5. All cross-references in body text and in the historical changelog entries below have been updated to the new numbering so the doc reads consistently for a fresh student. Phase 0 (Homebrew + Git) is unchanged.
- **Pre-Flight Check 4 relaxed.** Was a hard stop pointing students to https://ollama.com for the GUI installer. Now consistent with Pre-Flight Check 6 (Git): it surfaces a missing Ollama early but is no longer a hard stop, because the new Phase 1 will install it via Homebrew shortly afterwards.
- **No source-code changes.** `config.py`, `src/*.py`, and `requirements.txt` are untouched. `OLLAMA_BASE_URL = http://0.0.0.0:11434` is preserved exactly per the v1.4 multi-user-serving decision. *(Note for readers in v2.2 and later: at the time of v2.1 the project also shipped a separate `requirements.lock.txt`; that file was collapsed into a single `requirements.txt` in v2.2 — see top of changelog.)*

**Changes in v2.0 (fork from v1.7 — 2026-05-05):**
- **Hardware target retuned for 16GB Macs.** Minimum free disk lowered from 25GB to 15GB in Pre-Flight Check 3. Memory budget rewritten throughout from "comfortable on 64GB" to "tight but workable on 16GB".
- **Chat + vision model swapped** — `CHAT_MODEL` and `VISION_MODEL` are now `gemma4:e4b` instead of `gemma4:26b`. The 2-model design (embedder + multimodal LLM) is preserved unchanged.
- **Phase 2 simplified** — pulls `nomic-embed-text` and `gemma4:e4b`. The `gemma4:26b` reference from v1's historical 2-model decision is removed from the active steps (the v1.4 changelog block below preserves the original decision history for context). *(Was Phase 1 in v2.0; renumbered to Phase 2 in v2.1.)*
- **New Pre-Flight Check 7** — RAM verification (`sysctl hw.memsize`) added so a fresh-install runner confirms ≥16GB before they attempt the install.
- **New Common Pitfalls entry** — Pitfall 9: swap thrashing / unresponsive UI on 16GB machines. Symptoms, diagnosis with Activity Monitor / `vm_stat`, and mitigations (close other apps, drop `OLLAMA_KEEP_ALIVE` to `5m`, accept slower per-query latency).
- **Vision quality disclosure** added to the Architecture Overview and Phase 8 Ingestion sections — students should see the trade-off explicitly, not discover it later.
- **U-28 (two-tier model strategy) marked N/A** in v2's `UPDATES_NEEDED.md` — the chat model is already small and fast enough that the two-tier rationale doesn't apply. v1's open improvements list otherwise carries over identically.

**Changes in v1.7 (Git tooling — 2026-05-05):**
- **Phase 0 expanded** — added **Step 0.4 (Check Git)**, **Step 0.5 (Install Git via Homebrew with `brew install git`)**, and **Step 0.6 (Verify Git)** after the existing Homebrew install and verify steps. Git is the industry-standard version control system — needed for shipping the project to the future production VPS, for the occasional pip dependency that fetches a specific commit from a Git URL, and for keeping a forensic history of buildout changes going forward. macOS often ships a bundled copy of Git via the Xcode Command Line Tools, but the Homebrew-managed copy is preferable: it stays on the same upgrade cycle as Python and the rest of the toolchain, and `brew upgrade git` works whenever you want a newer version.
- **Pre-Flight Check 6 added** — a `git --version` line was added to the Pre-Flight Checks section so a fresh-install runner discovers a missing Git *before* reaching Phase 0 instead of during it. Failing this check is not a hard stop — Phase 0 Step 0.5 will install Git regardless — but knowing now means the install is deliberate, not a surprise.

**Changes in v1.6 (fresh-install hardening — 2026-05-03):**
- **Defensive transitive pins (Tier 1)** — added `pydantic==2.12.3`, `pydantic_core==2.41.4`, and `numpy==1.26.4` as direct pins in `requirements.txt`. These are technically transitive dependencies (LangChain pulls them in), but version drift in any of them has historically caused breaking changes — most recently the gradio_client/Pydantic schema crash that bit us during Phase 10 execution. Explicit pinning insulates fresh installs from accidental upgrades.
- **Lockfile-first install (Tier 2)** — Phase 6 Step 6.2 installs from a fully-pinned dependency file containing all 178+ packages (direct + transitive) at the exact versions verified working on 2026-05-03. Pip's resolver doesn't get to make any choices, eliminating ~99% of "could break in the future" risk for fresh installs. *(Originally landed as a separate `requirements.lock.txt` alongside a curated `requirements.txt` "shopping list"; collapsed into a single `requirements.txt` in v2.2 — see top of changelog. The hardening principle is unchanged; only the file layout changed.)*
- **Pre-Flight Checks section (Tier 3a)** — new short section inserted before Phase 0 verifying Python 3.12, disk space, Ollama installed and running, and internet reachability. Catches environmental issues *before* the maintainer commits to a 15-minute `pip install` that's going to fail at the end.
- **Common Pitfalls section (Tier 3b)** — new troubleshooting reference inserted between Phase 11 and Phase 12. Documents every gotcha encountered during the original buildout (venv not active, ChromaDB telemetry warnings, Ollama connection refused, Pydantic keep_alive validation error, etc.) with symptom → cause → fix entries.

**Changes in v1.5.1 (post-execution fixes — 2026-05-03):**
- **OllamaEmbeddings keep_alive (Known Fix backport — both heredocs)** — removed `keep_alive=config.OLLAMA_KEEP_ALIVE,` from the `OllamaEmbeddings(...)` block in BOTH the Phase 8 `ingest.py` heredoc AND the Phase 9 `chat.py` heredoc. The Phase 8 Known Fix (langchain-ollama 0.2.2 Pydantic `extra_forbidden`) was applied to `src/ingest.py` (the live file) during initial Phase 8 testing but was never backported into the heredocs — so `chat.py` (Phase 9) reproduced the same bug at module load with `pydantic_core._pydantic_core.ValidationError: ... Extra inputs are not permitted`, blocking every `import src.chat` and therefore `src/app.py` startup too. `ChatOllama` keeps `keep_alive` correctly (it accepts the field) — that line is untouched in Phase 9. The direct `ollama` client used in Phase 8's `describe_image()` also keeps `keep_alive` (also legitimate). `src/chat.py` was patched manually during execution; this v1.5.1 edit syncs both heredocs with the live files so future regenerations don't reintroduce the bug.
- **Gradio upgrade (5.13.0 → 5.50.0, gradio_client → 1.14.0)** — bumped pinned versions in Phase 6 / `requirements.txt`. Required to fix a `gradio_client/utils.py` schema-parser crash (`TypeError: argument of type 'bool' is not iterable` at `if "const" in schema:`) caused by Pydantic 2.13.3 emitting `additionalProperties: True` schemas that `gradio_client` 1.6.0 couldn't handle. Fixed in `gradio_client` 1.7.0+; we landed on 1.14.0 by upgrading `gradio` itself to the latest 5.x. Added explicit `gradio_client==1.14.0` pin to `requirements.txt` (defensive — was previously an unpinned transitive dependency, which is exactly how this bug bit us).
- **Phase 10 launch error message decoded** — Gradio's `When localhost is not accessible, a shareable link must be created. Please set share=True...` error after the schema crash is a misleading downstream symptom, NOT a real localhost reachability problem. **Do not set `share=True`** — that would expose an enterprise knowledge base to the public internet. The schema-parser fix above eliminates the underlying crash and the misleading message disappears with it.

**Changes in v1.5 (conversation memory — 2026-05-02):**
- **HISTORY_TURNS** — added `HISTORY_TURNS = 6` to `config.py` (Retrieval Settings section). Controls how many prior messages are injected into the prompt for follow-up-question handling.
- **Prompt template** — `RAG_PROMPT_TEMPLATE` in `chat.py` now includes a `{history}` block and explicit instructions to use prior exchanges to resolve pronouns and follow-up questions.
- **`format_history()` helper** — new helper in `chat.py` converts Gradio's messages-format history list into a plain-text conversation block, trimmed to `HISTORY_TURNS`. Filters out the empty assistant placeholder defensively.
- **LCEL chain** — `qa_chain` gains a third branch `"history": itemgetter("history")` so the prompt's three input variables (`history`, `context`, `question`) are all populated.
- **`query()` and `query_stream()`** — both signatures now accept `history: list = None`. `query_stream()` also logs the formatted history block at DEBUG level for forensic verification.
- **`chat_handler` in `app.py`** — passes `history[:-2]` (the prior turns, excluding the just-appended user message and empty assistant placeholder) into `query_stream()`. Conversation memory is per-browser-tab only; the server never remembers it between sessions.

**Changes in v1.2 (from UPDATES_NEEDED.md Sections 1 & 2):**
- **U-02** — Added `src/__init__.py` creation to Phase 4 Step 4.2 (fixes `ModuleNotFoundError: No module named 'src'`)
- **U-03** — Replaced Docling converter init and image-extraction loop with correct pipeline options and traversal pattern; added post-ingest image assertion
- **U-06** — Made ingestion idempotent: deterministic chunk IDs prevent duplicates; processed files move to `ingest/processed/` automatically
- **U-09** — Pinned all package versions in `requirements.txt`; added `pip freeze` lockfile step
- **U-09b** — Added startup version banner (`log_versions()`) to `ingest.py` and `app.py`
- **U-10** — Added `OLLAMA_KEEP_ALIVE = "24h"` to `config.py`; passed `keep_alive` to all Ollama clients to prevent model unloading mid-ingest
- **U-18** — Added shared rotating-file logger (`src/logging_setup.py`) as new Phase 7.5; replaced `print()` with `log` calls throughout all scripts
- **U-19** — Moved hardcoded `context[:500]` to `VISION_CONTEXT_CHARS = 500` in `config.py`

**Changes in v1.4 (U-ARCH architectural decision — 2026-05-01):**
- **U-ARCH RESOLVED** — Collapsed from 3-model to 2-model design. `llama3.2-vision:11b` removed entirely. `gemma4:26b` is multimodal and handles both image description (ingest-time) and chat responses. Saves ~8GB RAM, eliminates one `ollama pull`, and simplifies the code path. `VISION_MODEL` in `config.py` is now an alias for `CHAT_MODEL`.
- **OLLAMA_BASE_URL confirmed** — `http://0.0.0.0:11434` is intentional: Ollama is configured to serve all users on the Mac Mini M4. U-11 (change to localhost) does not apply to this setup.

**Changes in v1.3 (from UPDATES_NEEDED.md Sections 3 & 4):**
- **U-17** — Raised `CHUNK_SIZE` 1000 → 1800 and `CHUNK_OVERLAP` 150 → 200 in `config.py`; larger chunks keep multi-step procedures and spec tables intact in a single retrieval unit
- **U-16** — Replaced single-stage `RecursiveCharacterTextSplitter` with a two-stage pipeline in `ingest.py`: `MarkdownHeaderTextSplitter` first (preserves h1/h2/h3 as chunk metadata), then `RecursiveCharacterTextSplitter` within each section
- **U-07** — Replaced deprecated `RetrievalQA.from_chain_type` with an LCEL chain in `chat.py`; eliminates deprecation warnings and unlocks native `.stream()` support
- **U-05** — Added `query_stream()` generator to `chat.py`; rewrote `chat_handler` in `app.py` as a streaming generator using `yield` so tokens appear word-by-word in the UI
- **U-08** — Added `type="messages"` to `gr.Chatbot()` in `app.py`; switches history format from deprecated `[user, assistant]` tuples to `{"role":..., "content":...}` dicts (Gradio 5 standard)
...
## Pre-Flight Checks (Run These BEFORE You Start)

Seven quick environmental checks. They take 2 minutes and catch the most common reasons a fresh install fails. Run each command and verify the output matches the **Expected** description before moving on. If any check fails, fix it before proceeding — don't just hope it'll work out.

> **About copy-paste:** every command in this guide is meant to be copied as-is into a terminal. The lines starting with `#` are comments — they are part of the copy-paste and pip/bash ignores them automatically. The lines starting with `$` (when you see them in expected output) are NOT part of the input — they represent the shell prompt.

**Check 1 — Verify operating system and architecture:**

```bash
uname -sm
```
> **What it does:** `uname` prints information about the operating system. The flags `-s` and `-m` together print the OS kernel name and the hardware architecture.
>
> **Expected on Mac Mini M4:** `Darwin arm64` (Darwin = the macOS kernel; arm64 = Apple Silicon).
>
> **Expected on Linux x86_64 server (future VPS):** `Linux x86_64`.
>
> **Why it matters:** Some of our dependencies (notably PyTorch and ChromaDB's compiled extensions) ship different prebuilt binaries for Apple Silicon vs Intel/AMD. Knowing your architecture upfront prevents a confusing "no matching wheel" error 12 minutes into the install.

**Check 2 — Verify Python 3.12 is the active version:**

```bash
python3 --version
```
> **What it does:** Asks Python to print its own version number.
>
> **Expected:** `Python 3.12.x` (where `x` is any patch number, e.g. 3.12.13).
>
> **If you see Python 3.11 or 3.13:** Stop here. This project's pinned packages were tested against 3.12 specifically. Newer versions may work but introduce risk. Older versions may be missing features the code uses. Install Python 3.12 first via Homebrew (Phase 3 covers this).
>
> **If you see "command not found":** Python is not installed. Continue to Phase 0 — Homebrew first, then Phase 3 will install Python 3.12.

**Check 3 — Verify enough free disk space:**

```bash
df -h /
```
> **What it does:** `df` = "disk free". `-h` makes it print sizes in human-readable units (GB, MB) instead of raw byte counts. The `/` argument asks about the root filesystem.
>
> **Expected:** the `Avail` column should show **at least 15 GB free**. The chatbot itself needs ~13 GB (gemma4:e4b model is ~10 GB, nomic-embed-text ~0.3 GB, Python venv + dependencies ~2 GB, ChromaDB + ingested documents ~0.5 GB to start, working space for pip downloads). 15 GB gives a small buffer.
>
> **If you have less than 15 GB:** clear space before continuing. Pip will fail mid-install if it runs out of disk, and the partial state can be confusing to clean up. On macOS, `Apple > About This Mac > More Info > Storage` shows what's using space. Cleaning Downloads and the Trash usually frees a surprising amount.

**Check 4 — Verify Ollama is installed and the daemon is reachable:**

```bash
ollama list
```
> **What it does:** Asks the running Ollama daemon to list every model currently installed. This is also the simplest way to confirm Ollama is both installed AND its background service is running.
>
> **Expected:** A table of models. After Phase 2 completes, this table should include `gemma4:e4b` and `nomic-embed-text`. On a brand-new install, the table may be empty (just the header row) — that is fine; Phase 2 will populate it.
>
> **If you see "command not found":** Ollama isn't installed. This is **not a hard stop** for the pre-flight — Phase 1 (Install and Start Ollama) will install it via Homebrew (`brew install ollama`) shortly. Knowing it's missing now just means the install in Phase 1 is deliberate rather than a surprise. Mirrors how Pre-Flight Check 6 handles a missing Git.
>
> **If you see "Error: could not connect to ollama app":** Ollama is installed but the background daemon isn't running. Phase 1 Step 1.3 will start it with `brew services start ollama` (which also auto-starts it on every future login). If you want to start it right now without waiting for Phase 1, run that command. On Linux, the equivalent is `systemctl start ollama`.

**Check 5 — Verify internet connectivity to PyPI (Python's package repository):**

```bash
curl -I https://pypi.org
```
> **What it does:** `curl` is a command-line HTTP client. `-I` tells it to fetch only the HTTP headers (the metadata) rather than the full page contents — this is a fast way to confirm a server is reachable without downloading anything.
>
> **Expected:** A response starting with `HTTP/2 200` (or `HTTP/1.1 200`), followed by some response headers. The exact headers don't matter — `200` means success.
>
> **If you see "Could not resolve host":** Your machine doesn't have working DNS or internet access. Pip cannot download packages without internet — fix the network problem before continuing. Common causes: VPN that's blocking corporate proxies, firewall rules blocking outbound HTTPS, captive Wi-Fi portal that hasn't been logged into.

**Check 6 — Verify Git is installed:**

```bash
git --version
```
> **What it does:** Asks Git to report its installed version number. Git is the version control system we'll use to ship the project to the VPS later, and a few pip dependencies occasionally need it on the PATH to fetch a specific commit from a Git URL.
>
> **Expected:** A line like `git version 2.45.x` (any 2.x version is fine). On macOS, Git often comes bundled with the Xcode Command Line Tools, so it may already be present on a fresh machine.
>
> **If you see "command not found":** Git isn't installed. This is **not a hard stop** for the pre-flight — Phase 0 Step 0.5 will install Git via Homebrew (`brew install git`) shortly after Homebrew itself goes in. Knowing it's missing now just means the install in Phase 0 is deliberate rather than a surprise.

**Check 7 — Verify the machine has at least 16 GB of RAM (v2-specific):**

```bash
echo "$(($(sysctl -n hw.memsize) / 1024 / 1024 / 1024)) GB"
```
> **What it does:** `sysctl hw.memsize` asks the macOS kernel to report total physical RAM in bytes. The shell arithmetic `$(( ... / 1024 / 1024 / 1024 ))` divides by 1024 three times to convert bytes → kilobytes → megabytes → gigabytes. The whole thing is wrapped in `echo "... GB"` so the result is printed with the unit. (On Linux the equivalent is `grep MemTotal /proc/meminfo` — different command, same idea.)
>
> **Expected:** **`16 GB` or more**. Anything less is a hard stop for v2 — Gemma 4 e4b alone wants ~10–12 GB during inference, plus the embedding model, Python, ChromaDB, the OS itself, and whatever else you have open. On 8 GB the system will swap continuously and inference latency becomes unusable.
>
> **If you see less than 16 GB:** v2 is not the right project for this machine. Two options. (1) If the machine you actually plan to use is 16GB+ and you're just running pre-flight on a different one, that's fine — re-run on the target machine. (2) If 8 GB is what you have, this RAG project (any version) is not a good fit; consider a remote-LLM-only architecture instead (no local Gemma 4) where the laptop only runs the embedder + ChromaDB + Gradio, and chat inference goes to AWS Bedrock or Azure OpenAI. The `Enterprise_architecture.md` document in this project describes that production layout.

**All seven checks passed?** Proceed to Phase 0. **Any check failed?** Fix it first (Check 6 excepted — that one gets fixed inside Phase 0; Check 7 is a hard stop). Every minute spent here saves an hour of debugging later.
...
## Phase 11 — Run and Test the System

**Step 11.1 — Start a new terminal as root and activate the environment**

```bash
cd /Users/andrew-m4/Documents/Claude/Projects/Chatbotv2/Chatbotv2
source chatbot-16G-venv/bin/activate
```
...
## Appendix C — What We Deliberately Did NOT Include (Yet)

| Feature | Reason deferred |
|---|---|
| YouTube ingestion | Not in scope for this stage |
| Scheduling / callout booking | Build Stage 5 — requires calendar/spreadsheet integration |
| Voice interface | Production feature — the architecture uses HTTP API calls throughout, so a future STT (speech-to-text) input layer and TTS (text-to-speech) output layer can be added without changing the RAG core. The single prerequisite for a smooth voice experience — response streaming — is now implemented (U-05). |
| Docker containerisation | Not required for local PoC; skills transfer when ready |
| Authentication / login | PoC only — add before any production deployment |
| Multi-user support | Gradio supports it; deferred until needed |

---

*Document version: 2.1 — 2026-05-06*  
*Updates applied: U-02, U-03, U-05, U-06, U-07, U-08, U-09, U-09b, U-10, U-16, U-17, U-18, U-19 (UPDATES_NEEDED.md Sections 1–4) + U-ARCH (2-model design)*  
*v2 changes: chat/vision model swap to `gemma4:e4b` (v2.0); explicit Ollama install + service phase added between Phase 0 and the model-pull phase, all subsequent phases shifted up by 1 (v2.1).*  
*Next step on the target 16GB Mac: Run Pre-Flight Checks → Phase 0 (Homebrew + Git) → Phase 1 (Install + Start Ollama) → Phase 2 (Pull models: `nomic-embed-text` + `gemma4:e4b`) → Phase 3 (Python 3.12) → ... → Phase 11 (Run & Test).*
